...
// Carros que SOBEM e DESCEM + personagem animado
// ============================================
...
function startGame() {
    ...
    isPaused = false;
    playSound('start');
    messageEl.innerHTML = 'Você está correndo!  |  <strong>P / ESC</strong> = Pausar';
    gameLoop();
}
...
// Cria carro que DESCE ou que SOBE
function spawnObstacle() {
    ...
    // 55% desce, 45% sobe
    const goingDown = Math.random() < 0.55;

    let startY, dir;
    if (goingDown) {
        startY = -OBSTACLE_HEIGHT - 30; // começa em cima
        dir = 1; // desce
    } else {
        startY = canvas.height + 30;    // começa embaixo
        dir = -1; // sobe
    }
...
function updateObstacles() {
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        // Saiu por baixo (desceu)
        if (obs.dir === 1 && obs.y > canvas.height + 40) {
        ...
        // Saiu por cima (subiu)
        else if (obs.dir === -1 && obs.y + obs.height < -40) {
            obstacles.splice(i, 1);
            score += 15;
            scoreEl.textContent = 'Pontos: ' + score;
            playSound('score');
...
function gameLoop() {
    ...
    if (frameCount % Math.max(18, 55 - Math.floor(speed * 2.2)) === 0) {
        spawnObstacle();
    }
...
function drawStartScreen() {
    drawRoad();
    drawPlayer();
    messageEl.innerHTML = 'Pressione <strong>ESPAÇO</strong> ou qualquer <strong>SETA</strong> para começar!<br>Carros sobem e descem na estrada';
}
