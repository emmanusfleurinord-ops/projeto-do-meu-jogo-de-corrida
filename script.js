// ============================================
// CORRIDA F-E-K
// Carros sobem E descem + Tela Cheia (tecla F)
// ============================================
...
let runFrame = 0;
let bodyBounce = 0;
let spawnCount = 0; // para garantir os dois sentidos
...
// ===== AQUI ESTÁ A CORREÇÃO DOS CARROS =====
function spawnObstacle() {
    ...
    // Alterna de forma garantida: um desce, outro sobe, um desce, outro sobe...
    // Assim sempre terá os dois tipos
    const goingDown = (spawnCount % 2 === 1);

    let startY, dir;

    if (goingDown) {
        // CARRO QUE DESCE (de cima para baixo)
        startY = -OBSTACLE_HEIGHT - 40;
        dir = 1;
    } else {
        // CARRO QUE SOBE (de baixo para cima)
...
function updateObstacles() {
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (obs.dir === 1 && obs.y > canvas.height + 50) {
            // Desceu e saiu por baixo
            obstacles.splice(i, 1);
            score += 15;
            scoreEl.textContent = 'Pontos: ' + score;
            playSound('score');
        }
        else if (obs.dir === -1 && obs.y + obs.height < -50) {
            // Subiu e saiu por cima
            obstacles.splice(i, 1);
            score += 15;
            scoreEl.textContent = 'Pontos: ' + score;
            playSound('score');
...
function drawStartScreen() {
    drawRoad();
    drawPlayer();
    messageEl.innerHTML = 'Pressione <strong>ESPAÇO</strong> ou qualquer <strong>SETA</strong> para começar!<br><strong>F</strong> = Tela Cheia  |  Carros sobem e descem';
}
