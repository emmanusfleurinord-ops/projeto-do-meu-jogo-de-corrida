// ============================================
// CORRIDA F-E-K
// Estrada com 3 faixas + Árvores dos lados
// ============================================
...
function playSound(type) {
    try {
        ...
        if (type === 'start') {
            ...
            osc.frequency.linearRampToValueAtTime(1100, audioCtx.currentTime + 0.07);
            ...
            if (now - lastPassSoundTime < 0.28) return;
            lastPassSoundTime = now;
...
const obstacleColors = ['#f1c40f', '#e67e22', '#3498db', '#e74c3c', '#9b59b6', '#1abc9c', '#e91e63'];
...
function updateObstacles() {
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (!obs.passed && obs.y > player.y + player.height + 10) {
            obs.passed = true;
            playSound('pass');
            score += 15;
...
function drawRoad() {
    ...
    // Linhas das faixas (brancas pontilhadas)
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 4;
    ctx.setLineDash([18, 22]);
    ctx.lineDashOffset = -roadOffset;

    // Faixa do meio esquerda
    ctx.beginPath();
    ctx.moveTo(LANE_WIDTH + 20, 0);
    ctx.lineTo(LANE_WIDTH + 20, canvas.height);
    ctx.stroke();

    // Faixa do meio direita
    ctx.beginPath();
    ctx.moveTo(LANE_WIDTH * 2 - 20, 0);
    ctx.lineTo(LANE_WIDTH * 2 - 20, canvas.height);
    ctx.stroke();
    ...
    // ===== ÁRVORES DOS LADOS =====
    // As árvores se movem junto com a estrada
    const treeSpacing = 90;
    const offset = roadOffset % treeSpacing;

    for (let i = -1; i < canvas.height / treeSpacing + 2; i++) {
        const ty = i * treeSpacing - offset;

        // Árvores do lado ESQUERDO
        drawTree(22, ty);

        // Árvores do lado DIREITO
        drawTree(canvas.width - 22, ty);
    }
}
...
function drawObstacles() {
    for (let obs of obstacles) {
        ...
        roundRect(ctx, x + 4, y + 8, w - 8, h - 16, 10);
        ctx.fill();

        // Cabine
        ctx.fillStyle = shadeColor(obs.color, -40);
        roundRect(ctx, x + 10, y + 14, w - 20, h - 38, 6);
        ctx.fill();

        // Janela
        ctx.fillStyle = 'rgba(30, 30, 50, 0.7)';
        roundRect(ctx, x + 13, y + 17, w - 26, h - 48, 4);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(x + 14, y + h - 14, 5, 0, Math.PI * 2);
        ctx.arc(x + w - 14, y + h - 14, 5, 0, Math.PI * 2);
        ctx.fill();

        // Rodas
        ctx.shadowBlur = 0;
        ctx.fillStyle = '#111';
        ctx.fillRect(x + 6, y + 12, 8, 12);
        ctx.fillRect(x + w - 14, y + 12, 8, 12);
        ctx.fillRect(x + 6, y + h - 24, 8, 12);
        ctx.fillRect(x + w - 14, y + h - 24, 8, 12);
    }
    ctx.shadowBlur = 0;
}
